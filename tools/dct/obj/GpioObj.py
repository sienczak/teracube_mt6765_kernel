...
class GpioObj(ModuleObj):
    ...
    def get_cfgInfo(self):
        ...
        for op in ops:
            ...
            data = GpioData()
            data.set_smtNum(atoi(list[len(list)-1]))
        ...
        for node in nodes:
            if node.nodeType == xml.dom.Node.ELEMENT_NODE:
                if cmp(node.nodeName, 'count') == 0:
                    GpioData._count = atoi(node.childNodes[0].nodeValue)
                    continue
                ...
                if len(defmNode):
                    data.set_defMode(atoi(defmNode[0].childNodes[0].nodeValue))
...
class GpioObj_MT6739(GpioObj_MT6759):
    ...
    def get_eint_index(self, gpio_index):
        if atoi(gpio_index) in GpioData._map_table.keys():
            return GpioData._map_table[atoi(gpio_index)]
        return -1
