...
class KpdObj(ModuleObj):
    ...
    def get_cfgInfo(self):
        ...
        for op in ops:
            KpdData._keyValueMap[op.upper()] = atoi(cp.get('Key_definition', op))

        KpdData._keyValueMap['NC'] = 0

        cp.read(ModuleObj.get_figPath())
        if cp.has_option('KEYPAD_EXTEND_TYPE', 'KEY_ROW'):
            KpdData.set_row_ext(atoi(cp.get('KEYPAD_EXTEND_TYPE', 'KEY_ROW')))
        if cp.has_option('KEYPAD_EXTEND_TYPE', 'KEY_COLUMN'):
            KpdData.set_col_ext(atoi(cp.get('KEYPAD_EXTEND_TYPE', 'KEY_COLUMN')))
        ...
        for node in nodes:
            if node.nodeType == xml.dom.Node.ELEMENT_NODE:
                if node.nodeName == 'row':
                    row = atoi(node.childNodes[0].nodeValue)
                    KpdData.set_row(row)

                if node.nodeName == 'column':
                    col = atoi(node.childNodes[0].nodeValue)
                    KpdData.set_col(col)
                ...
                if node.nodeName == 'pwrKeyEint_gpioNum':
                    num = atoi(node.childNodes[0].nodeValue)
                ...
                if node.nodeName == 'pressPeriod':
                    time = atoi(node.childNodes[0].nodeValue)
