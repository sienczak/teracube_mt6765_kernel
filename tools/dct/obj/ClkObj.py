...
class ClkObj(ModuleObj):
    ...
    def get_cfgInfo(self):
        ...
        count = atoi(cp.get('CLK_BUF', 'CLK_BUF_COUNT'))
        self.__count = count

        ops = cp.options('CLK_BUF')
        for op in ops:
            if op == 'clk_buf_count':
                self.__count = atoi(cp.get('CLK_BUF', op))
                ClkData._count = atoi(cp.get('CLK_BUF', op))
                continue
            ...
            data.set_defVarName(atoi(var_list[0]))
            data.set_defCurrent(atoi(var_list[1]))
