...
from utility.py2to3 import atoi, cmp
...
class EintObj(ModuleObj):
    ...
    def get_cfgInfo(self):
        ...
        for op in ops:
            ...
            map[atoi(re.findall(r'\d+', op)[0])] = atoi(list[len(list)-2])
        ...
        if cp.has_option('EINT', 'EINT_MAP_COUNT'):
            self.__map_count = atoi(cp.get('EINT', 'EINT_MAP_COUNT'))
    ...
    def fill_mappingTable(self):
        ...
        if self.__map_count == 0:
            for i in range(0, atoi(self.__count)):
    ...
    def refGpio(self, eint_num, flag):
            ...
            if atoi(eint_num) == atoi(key):
                ...
                if flag:
                    for item in temp_map.keys():
                        item_data = self.__gpio_obj.get_gpioData(atoi(item))

                        if item_data.get_defMode() == atoi(temp_map[item].split(':')[0]):
                            gpio_vec = []
        ...
        gpio_num = EintData.get_gpioNum(atoi(eint_num))
        if gpio_num >= 0:
    ...
    def fill_dtsiFile(self):
        ...
        for key in sorted_list:
            ...
            gen_str += '''\tdebounce = <%s %d>;\n''' %(self.refGpio(key[4:], True)[0], atoi(value.get_debounceTime()) * 1000)
            gen_str += '''\tstatus = \"okay\";\n'''
            gen_str += '''};\n'''
            gen_str += '''\n'''
    ...
    def fill_dtsiFile(self):
        ...
        for key in sorted_list:
            ...
            if cmp(value.get_debounceEnable(), 'Enable') == 0:
                ...
                gen_str += '''\tdebounce = <%d>;\n''' % (atoi(value.get_debounceTime()) * 1000)
            gen_str += '''\tstatus = \"okay\";\n'''
            gen_str += '''};\n'''
            gen_str += '''\n'''
    ...
    def refGpio_defMode(self, eint_num, flag):
        ...
        for key in EintData._builtin_map.keys():
            if atoi(eint_num) == atoi(key):
                ...
                if flag:
                    for item in temp_map.keys():
                        item_data = self.get_gpioObj().get_gpioData(atoi(item))
